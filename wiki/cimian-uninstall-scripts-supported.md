...
- **Uninstall command**: `Remove-AppxProvisionedPackage -Online -PackageName <PackageFullName>`
...
- **Known limitation**: Provisioned packages are visible to **new** user profiles on
  next login. Currently-signed-in users must log out/back in to see the app. This
  matches Intune's MSIX deployment model.
- **Out of scope**: License files (`.xml`) required by Store-signed apps (use
  `preinstall_script` to import them), sideloading certificate installation, and
  per-architecture nested manifest extraction from `.msixbundle` payloads
  (bundle install works transparently; only top-level bundle identity is extracted
  into metadata).
