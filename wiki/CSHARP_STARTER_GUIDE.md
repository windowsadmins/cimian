...
> **Note:** The structure below reflects the **current** layout as of December 2024.  
> See `PROJECT_STRUCTURE.md` for the authoritative reference.

The `csharp` branch uses the following directory structure:

```
CimianTools/
├── CimianTools.sln                      # Main solution file
├── Directory.Build.props                # Common MSBuild properties
│
├── shared/                              # Shared libraries
│   ├── core/                            # Cimian.Core - Domain models
│   │   ├── Cimian.Core.csproj
│   │   ├── Models/
│   │   ├── Configuration/
│   │   ├── Logging/
│   │   └── Services/
│   ├── engine/                          # Cimian.Engine - Business logic
│   │   ├── Cimian.Engine.csproj
│   │   ├── Predicates/
│   │   ├── Manifest/
│   │   └── Package/
│   └── infrastructure/                  # Cimian.Infrastructure - External I/O
│       ├── Cimian.Infrastructure.csproj
│       ├── Download/
│       ├── Installers/
│       └── Process/
│
├── cli/                                 # Command-line tools (10 total)
...
│   ├── cimiwatcher/                     # Windows service
│   └── cimistatus/                      # CLI status tool
│
├── apps/                                # GUI applications
│   ├── ManagedSoftwareCenter/           # End-user self-service (WPF)
│   └── CimianStatus/                    # Admin status UI (WPF)
│
...
### Step 2: Create Solution File [x] COMPLETE

The solution file `CimianTools.sln` is located at the repository root and organizes projects into solution folders:

```
Solution 'CimianTools'
├── 📁 shared
│   ├── Cimian.Core
│   ├── Cimian.Engine
...
### Step 3: Directory.Build.props [x] COMPLETE

The `Directory.Build.props` file is located at the repository root and provides common MSBuild properties for all projects:
```xml
<Project>
  <PropertyGroup>
...
### Step 4: Central Package Management [x] COMPLETE

Package versions are centrally managed. Example from `Directory.Packages.props`:
```xml
<Project>
  <PropertyGroup>
...
### Step 5: Core Models [x] COMPLETE

Example from `shared/core/Models/CatalogItem.cs`:
```csharp
