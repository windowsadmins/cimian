...
// identifyRemovals enumerates all items under HKLM\Software\ManagedInstalls
// and returns any item that is NOT in the localCatalogMap (i.e., "no longer needed").
func identifyRemovals(localCatalogMap map[string]catalog.Item, _ *config.Configuration) []catalog.Item {
	...
	if err != nil {
		logging.Warn("Failed to read subkey names for removals", "error", err)
		return toRemove
	}

	for _, itemName := range subKeys {
		keyLC := strings.ToLower(itemName)
		// If not found in localCatalogMap => we consider it "stale" => remove
		if _, found := localCatalogMap[keyLC]; !found {
			logging.Info("Identified stale item for removal", "item", itemName)
			// Minimal catalog.Item with just .Name is enough to uninstall
			toRemove = append(toRemove, catalog.Item{Name: itemName})
		}
	}
...
func cleanManifestsCatalogsPreRun(dirPath string) error {
	...
	logger.Debug("Cleaned and recreated directory: %s", dirPath)
	return nil
}
