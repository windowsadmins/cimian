...
public class PredicateEngineTests
{
    ...
    [InlineData("hostname DOES_NOT_CONTAIN 'Camera'", "Camera-01", false)]
    ...
    public async Task EvaluateCondition_DoesNotContainWithAnd_ShouldMatch()
    {
        ...
        var condition = "hostname DOES_NOT_CONTAIN 'Camera' AND hostname DOES_NOT_CONTAIN 'DEPT-CAM'";
        
        var facts1 = CreateFacts(hostname: "Cintiq22-01");
        var result1 = await _engine.EvaluateConditionAsync(condition, facts1);
        result1.Should().BeTrue("Cintiq22-01 should match (no Camera or DEPT-CAM)");
        
        var facts2 = CreateFacts(hostname: "Camera-01");
        var result2 = await _engine.EvaluateConditionAsync(condition, facts2);
        result2.Should().BeFalse("Camera-01 should NOT match (contains Camera)");
        
        var facts3 = CreateFacts(hostname: "CAM-05");
        var result3 = await _engine.EvaluateConditionAsync(condition, facts3);
        result3.Should().BeFalse("CAM-05 should NOT match (contains DEPT-CAM)");
    }
