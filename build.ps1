...
function Get-SigningCertThumbprint {
    ...
    $cert = Get-ChildItem Cert:\CurrentUser\My | Where-Object { $_.HasPrivateKey -and $_.Subject -like '*ExampleOrgU*' } |
       Sort-Object NotAfter -Descending | Select-Object -First 1
    
    if (-not $cert) {
        $cert = Get-ChildItem Cert:\LocalMachine\My | Where-Object { $_.HasPrivateKey -and $_.Subject -like '*ExampleOrgU*' } |
           Sort-Object NotAfter -Descending | Select-Object -First 1
    }
...
if ($Sign) {
    Test-SignTool
    if (-not $Thumbprint) {
        $Thumbprint = (Get-ChildItem Cert:\CurrentUser\My | Where-Object { $_.HasPrivateKey -and $_.Subject -like '*ExampleOrgU*' } |
       Sort-Object NotAfter -Descending | Select-Object -First 1 -ExpandProperty Thumbprint)
        if (-not $Thumbprint) {
            Write-Log "No valid ExampleOrgU certificate with a private key found - aborting." "ERROR"
            exit 1
        }
...
function signNuget {
    ...
    -CertificateSubjectName 'Enterprise Code Signing Certificate' `
    -Timestamper            $tsa
    if ($LASTEXITCODE) {
