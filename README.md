...
> **Note**: Cimian will be fully migrated to C# for a truly native Windows codebase, retiring all Go code in the process. The Go code originated from a project called Gorilla that served as a basis to start from but with over 1,300 commits between September 2024 and September 2025 the code base is not related anymore from the original. Cimian now has CLI tools that have parity with Munki for Windows for all core functionality. I am plannning to consider it at v1.0 when the C# rewrite is done but it is fully functioning! We are dog fooding Cimian internally wirh our fleet of 500+ Windows devices. **The 'C' in Cimian stands for C# after all!**
...
Cimian consists of a suite of command-line tools, services, and GUI applications that work together to provide a software deployment management solution:
